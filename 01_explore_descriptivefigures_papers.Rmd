...
#TODO: Add here part of asbtract: background, objective, methods, resutls, conclusions. Check out cloud tag for non-sense words
...
About `r round(after_cleanup/total_words * 100)` % (`after_cleanup`) of the words in all abstracts are considered stop words.
...
```{r Fig_Word_cloud,dpi=600,fig.width=7,fig.asp=0.85}
set.seed(1)
if (max(top_20_words$n) < 100) {
  minimum_occurence <- round(mean(top_20_words$n))
} else {
  minimum_occurence <-  min(top_20_words$n) # Or arbritary number like 100 
}
...
_Word cloud of 20 top words in abstracts_ 
...
```{r  top_words_table, echo=TRUE}
# for testing
# kable(top_20_words)
kable(top_20_words,
      caption = "Top20 words in the corpus",
      format = "html", booktabs = TRUE)

#TODO: some top words and non-stop-words should be taken out (eg. background, conclusion, a)  
```
