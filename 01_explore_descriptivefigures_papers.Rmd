...
As part of the text analysis of papers, including wordcloud and terms frequency analysis, read the full list of abstracts from the `papers` and process them to create a [tidy](https://www.jstatsoft.org/article/view/v059i10) data structure without [stop words](https://en.wikipedia.org/wiki/Stop_words). Reference book to text mining in tidy format: [Text Mining with R](https://www.tidytextmining.com/tfidf.html)
...
###### EDA ##########
eda_papers_words <- tidy_abstracts %>%
    unnest_tokens(word, abstract) %>%
    count(id, word, sort = TRUE) %>%
    ungroup()

eda_total_papers_words <- eda_papers_words %>% 
  group_by(id) %>% 
...
ggplot(slice(non_stop_words_per_abstract, 1:50), aes(id, num_words, fill=year)) + 
  geom_col(show.legend = FALSE) +
  labs(x = NULL, y = "# words", title="Top50 abstracts by # words, by year") +
