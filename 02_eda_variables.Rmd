...
#' Sync changes to other two columns, md_id adn md_desc
md_data_kp <- 
    md_data_kp %>% 
        separate(md, c("md_id","md_desc"), "-", remove = FALSE) %>%
...
kp_variability <- 
    md_data_kp %>%
      ggplot(aes(x=md_desc, y=number_cases, label=number_cases)) + #, fill=val_ass, group= val_ass)) +
        ...
        geom_text(aes(label=number_cases), size=2, family="American Typewriter") +
        # geom_text(aes, (label=  color), nudge_y = 0.5, nudge_x= 0,  size = 2) +
