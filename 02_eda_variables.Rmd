...
* In terms of variaty, we can say that the range of MDs addressed by interventions.
* Top3 mental disorders (`r top3`) remain significant over all years.
* 
...
Let's start simple: _How many apps are related to the MDs and subtypes?_ We use a treemap diagram to represente hierarchical data in the form of nested rectangles, the area of each corresponding to its numerical value. i.e. the count of apps

```{r plot_md_apps, echo=FALSE}
...
n_apps %>%
    treemap(index=c("md_desc","md_sub"),  #A list of your categorical variables
        ...
        title="Number of apps per subtype and main category of MDs", #Customize your title
...
sens_apps %>%
    ...
    select(`DSM-5 Mental Disorder` = md_desc,
           ...
           `GPS` = sens_gps,
           `Microphone` = sens_mic) %>%
    kable(format = "html", caption = "Apps", escape = F) %>%
