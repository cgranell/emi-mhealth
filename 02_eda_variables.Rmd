...
md_top3_byyear$md_reordered <- factor(md_top3_byyear$md, levels = c('all', '4-Depressive disorders', '26-Various', '5-Anxiety disorders'))
brks <-levels(md_top3_byyear$md_reordered)

md_top3_byyear$md_desc <- fct_relevel(md_top3_byyear$md_desc, c("All", "Depressive disorders", "Various", "Anxiety disorders"))

md_top3_byyear %>%
    ggplot(aes(x=year, y=number_cases)) +
    geom_area(aes(fill=md_desc), colour="black", size=.2, alpha=.4) +
    scale_fill_brewer(palette="Greens", breaks=levels(md_top3_byyear$md_desc)) +
    #geom_text(aes(label = number_cases), position= "stack", color= "blue", size = 3) +
    geom_vline(aes(xintercept = 2017), linetype = 'dashed', size = 0.5) +
    guides(fill=guide_legend(title="Top MDs")) +  # modify legend title
    theme(legend.position = "bottom")

####### TODO: reorganise levels
