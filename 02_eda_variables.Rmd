...
md_percent <- 
    all_data %>%
      group_by(md) %>%
      summarise(n = n()) %>%
      mutate(proportion = round(n/n_papers, 2)) %>%
      arrange(desc(n))
...
Note: The period 2013-2017 covers full years. 2118 only covers the first tow months (Bibliographic searches were conducted at the beginning of March 2018). 

Remarks:
* There is no clear temporal trend. Only that the last years attract more papers/cases.
...
md_by_year <- 
    all_data %>% 
      group_by(md, year) %>%  # first create counts for each group
      summarise(number_cases = n()) %>% 
      group_by(year) %>%
...
head(md_by_year,6) %>%
    ...
    geom_vline(aes(xintercept = 2017), linetype = 'dashed', size = 0.1)
...
Remarks:
* There is a sudden peak in 2016-2017 compared to 2013-2015. The former accounts for (`r percent(60/n_paper)`)
