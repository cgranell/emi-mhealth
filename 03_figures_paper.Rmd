...
Proportional (stacked) barchart: Distribution of papers per mental disorders, colored by assessment type. Note that we draw two bars per mental disorder: one represents no assessment at all, the other is a stacked bar to show the types of assessment.
...
cols = c("id", "md", "md_id", "md_desc", 
         ...
         "type", "year")
cols_sens <- c("sens_acc", "sens_gyr", "sens_gps", "sens_mic")
...
n_sens_gps <- all_data %>% select(sens_gps) %>% filter(sens_gps == "YES") %>% nrow()
n_sens_mic<- all_data %>% select(sens_mic) %>% filter(sens_mic == "YES") %>% nrow()
...
`r percent(n_sens/n_papers)` of papers use **at least** a sensing capability (N=`r n_sens`). The use of the acc and gyr is residual; gps and mic are more significant, but in a low proportion: gps = `r percent(n_sens_gps/n_papers)`, mic = `r percent(n_sens_mic/n_papers)`. Note that some papers use both type of sensors, so the sum of percentage per sensro exceeds theo total percentage of papers that use **at least** a sensing capability.
...
```{r key_plot_5_sens_bubblechart, echo=FALSE, dpi=600, fig.width=12, fig.height=14, fig.asp=0.65}
...
```
```{r key_plot_5_bubblechart, echo=FALSE, dpi=600, fig.width=12, fig.height=14, fig.asp=0.65}
data_kp_sens <-
    data_kp_sens %>%
...
data_kp_all <-
    bind_rows(data_kp_sens, data_kp_feat)
