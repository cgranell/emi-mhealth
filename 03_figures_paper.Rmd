...
get_palette <- colorRampPalette(brewer.pal(9, "Accent"))
...
kp_circularbarplot <- 
    ggplot(data_kp_apps3, aes(x=as.factor(id), y=number_apps, fill=md_desc)) +  
      ...
      ylim(-5,10) +
      #scale_fill_brewer(palette = "Accent", name="Mental\n disorders") +
      theme_minimal() +
      theme(
...
ggsave(plot = kp_circularbarplot, filename = kp_file_name, device = "png", path = here::here("figs"),
    scale = 1, width = 20, height = 20, units = "cm", dpi = "print") #300

#TODO: SOlve issues with palette and legend
```
