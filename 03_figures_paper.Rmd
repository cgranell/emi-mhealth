...
#' Replace existing factors levels with "other levels"
...
# all_data$val_asstype <- fct_other(all_data$val_asstype, drop = c("OTHERS", "EFFICACY", "EFFICACY AND OTHERS"),
#                                   other_level = c("USABILITY/USER EXPERIENCE", "EFFECT", "EFFECT AND \nUSABILITY/USER EXPERIENCE"))
# levels(all_data$val_asstype )[levels(all_data$val_asstype )=="OTHERS"] <- "USABILITY/USER EXPERIENCE"
# levels(all_data$val_asstype )[levels(all_data$val_asstype )=="EFFICACY"] <- "EFFECT"
# levels(all_data$val_asstype )[levels(all_data$val_asstype )=="EFFICACY AND OTHERS"] <- "EFFECT AND \nUSABILITY/USER EXPERIENCE"
...
data_kp_areachart$val_asstype <- fct_relevel(data_kp_areachart$val_asstype, 
...
#TODO: Añadir totales a la figura
data_kp_all_n <-
    bind_rows(data_kp_feat_n, data_kp_sens_n, data_kp_anal_n)

# data_kp_all_ordered$tech_type    

data_kp_all$tech_type <- as_factor(data_kp_all$tech_type)
data_kp_all$tech_type <- fct_relevel(data_kp_all$tech_type, 
                                     c( "feat_learn", "feat_prog", "feat_pers", "feat_ass", "feat_promp", "feat_hcp", 
                                       "feat_use", "feat_soc", "feat_ca", "feat_game",  "feat_vr", "feat_ar", 
                                       "sens_mic", "sens_gps", "sens_cam", "sens_acc", "sens_gyr",  
                                       "anal_ml", "anal_act", "anal_beh", "anal_sp"))


brks <- levels(data_kp_all$cat) 

kp_bubblechart <- 
    data_kp_all %>%
    ggplot(aes(x=md_desc, y=tech_type)) +
    geom_point(aes(colour=cat, size=number_cases), alpha=0.8, na.rm = TRUE, show.legend = FALSE) +
    geom_text(aes(label=number_cases), size=3, na.rm = TRUE) +
    scale_size_area( max_size=15) +
    # scale_size(range=c(1, 15)) +
    coord_flip() +
    labs(title="Mental disorder vs Technology", 
         subtitle = "Technology items ordered within each group",
         x="Mental disorder", 
         y="Technology", 
         caption="Source: authors") + 
    scale_color_brewer(palette="Set2", breaks=brks) +
...
kp_bubblechart <- 
kp_bubblechart +
    # annotations total of columns
    annotate("text", x = "Personality disorders", y = "feat_learn", fontface =2,
                 label = "80", color = "#FC8D62", size=3, hjust = 0.4, vjust = -1.8) +
    annotate("text", x = "Personality disorders", y = "feat_prog", fontface =2,
                 label = "65", color = "#FC8D62", size=3, hjust = 0.4, vjust = -1.8) +
    annotate("text", x = "Personality disorders", y = "feat_pers", fontface =2,
                 label = "58", color = "#FC8D62", size=3, hjust = 0.4, vjust = -1.8) +
    annotate("text", x = "Personality disorders", y = "sens_mic", fontface =2,
                 label = "12", color = "#8DA0CB", size=3, hjust = 0.4, vjust = -1.8) +
    annotate("text", x = "Personality disorders", y = "anal_ml", fontface =2,
                 label = "6", color = "#66C2A5", size=3, hjust = 0.4, vjust = -1.8)
