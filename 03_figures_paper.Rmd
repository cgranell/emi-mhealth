...
Circular plot (Example 3): Emphasis on links between apps, i.e., when the same app is used in distinct mental disorder.
...
kp_file_name <- here::here("figs", "table3.html")

data_kp_apps3 <- 
    all_data %>%
    group_by(app_name, md) %>%
    summarise(number_apps = n()) %>% 
    arrange(app_name, number_apps)
