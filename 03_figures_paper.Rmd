...
# lbls <- as.data.frame(round(unique(data_kp_barchart$proportion_sum*100,1)))
...
kp_barchart <- 
    data_kp_barchart %>%
      ggplot(aes(x=md_desc, y=proportion)) + 
        ...
        labs(title="Distribution of papers in % per mental disorder", 
             subtitle = paste0("(N=",n_papers, ")"),
             x="Mental disorders", 
             y="Percentatge", 
             caption="Source: authors") + 
        geom_segment(aes(x=md_desc, y=0, xend=md_desc, yend=proportion_sum), color="grey50") +
        geom_point(aes(y=proportion_sum), size=3, color="darkgrey") +
        # geom_text(data=lbls, color= "black", size=3) +        #TODO
        annotate("text", x = "Schizophrenia spectrum and other psychotic disorders", y = .18,
                 label = "TOP 6", color = "black", size = 3, hjust = -0.1, vjust = 1) +
        ...
        guides(fill=guide_legend(title="Assessment")) +  # modify legend title
        scale_fill_brewer(palette = "Set2") +
        scale_y_continuous(expand=c(0,0), labels=scales::percent_format(accuracy = 1), breaks=seq(0, 0.20, by=0.02), limits=c(0, 0.20)) +
        theme_minimal()  +
        # Legend: Top-Right Inside the Plot") 
        theme(legend.title = element_text(size=11), 
              legend.justification = c('right', 'top'),
