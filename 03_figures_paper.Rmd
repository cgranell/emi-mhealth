...
data_kp_sens$md_desc <- factor(data_kp_sens$md_desc, levels=unique(data_kp_sens$md_desc))  

data_kp_sens %>%
    ggplot(aes(x=md_desc, y=sens_type)) +
    geom_point(aes(colour=sens_type, size=number_cases)) + 
...
data_kp_feat <- 
    ...
    group_by(md_desc, feat_type)  %>%
    summarise(number_cases = n()) 

data_kp_feat$md_desc <- factor(data_kp_feat$md_desc, levels=unique(data_kp_feat$md_desc))  
