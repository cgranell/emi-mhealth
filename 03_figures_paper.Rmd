...
data_kp_sens <- 
    all_data %>%
    select(cols) %>%
    filter((sens_acc != "NOT KNOWN") & (sens_gyr != "NOT KNOWN") & (sens_gps != "NOT KNOWN") & (sens_mic != "NOT KNOWN"))  %>%
    arrange(md_desc)
    # group_by(md_desc, md_sub) %>%
    # summarise(number_apps = n())

n_sens <- nrow(data_kp_sens)

# data_kp_sens_2 <-
    data_kp_sens %>%
    select(id, sens_acc, sens_gyr, sens_gps, sens_mic) %>%
    gather(sens_acc, sens_gyr, sens_gps, sens_mic, key="sens_type",value="sens_enabled") %>%
    ggplot(aes(x=factor(sens_type))) +
    geom_bar(stat="count", aes(fill=factor(sens_enabled)))
    

data_kp_sens %>%
    select(id, md_desc, sens_acc, sens_gyr, sens_gps, sens_mic) %>%
    gather(sens_acc, sens_gyr, sens_gps, sens_mic, key="sens_type",value="sens_enabled") %>%
...
data_kp_sens2$md_desc <- factor(data_kp_sens2$md_desc, levels=unique(data_kp_sens2$md_desc))  
...
#TODO: geom_point - https://stackoverflow.com/questions/37687544/using-scale-size-area-ggplot2-to-plot-points-of-size-0-as-completely-absent
# Eliminar todos los NO, NOT KNOWN
# agrupar por mmd_desc & sens_type, y contar los "yes" en nueva columna  n() 

data_kp_sens2 %>%
    data_kp_sens2 %>%
    group_by(md_desc, sens_type) %>%


data_kp_sens2 %>%
    ggplot(aes(x=md_desc, y=sens_type)) +
    geom_jitter(aes(color = sens_type), size=5) +
    coord_flip()
