...
`r percent(n_sens/n_papers)` of papers use **at least** a sensing capability (N=`r n_sens`). The use of the acc and gyr is residual; gps and mic are more significant, but sitllt in a low proportion. gps = `r percent(table(data_kp_sens$sens_gps)[3]/n_papers)`, mic = `r percent(table(data_kp_sens$sens_mic)[3]/n_papers)`
...
data_kp_sens %>%
    ...
    geom_point(aes(colour=sens_type, size=number_cases), show.legend=FALSE, na.rm = TRUE) + 
...
data_kp_all %>%
    ...
    geom_point(aes(colour=cat, size=number_cases), show.legend=FALSE, na.rm = TRUE) + 
    geom_text(aes(label=number_cases), size=3, na.rm = TRUE) +
    scale_size_area(max_size=15) +
    coord_flip() +
