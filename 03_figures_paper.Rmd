...
```{r key_plot_1_barchart, echo=FALSE, dpi=600,fig.width=12,fig.asp=0.85}
#  Fig1,dpi=600,fig.width=7,fig.asp=0.85
data_kp_barchart <- 
    all_data %>%
...
#         group_by(md_desc) %>%
#         mutate(total_cases = sum(number_cases),
#                proportion_cases = number_cases/total_cases,
#                proportion_all = number_cases/n_papers) %>%
#         ungroup() %>%
...
```{r key_plot_1_barchart_bis, echo=FALSE,dpi=600,fig.width=10,fig.asp=0.85}

kp_barchart <- 
    data_kp_barchart %>%
      ggplot(aes(x=md_desc, y=proportion)) + #, )) +
        geom_bar(stat="identity", aes(fill=val_ass), position = "dodge") +
        # geom_text(aes(label=proportion_sum, group=val_ass), size=2, nudge_y=0.2) +
        labs(title="Distribution of papers in % per mental disorder", 
             subtitle = paste0("(N=",n_papers, ")"),
             x="Mental disorders", 
             y="Percentatge", 
             caption="Source: authors") + 
        geom_lollipop(point.colour = 'black', aes(y=proportion_sum), point.size = 3,horizontal = FALSE) +
