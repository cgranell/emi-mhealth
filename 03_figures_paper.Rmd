...
plot_topmd <-
    data_plot_topmd %>%
        ...
        geom_point(shape=21, size=6, color="darkgray", fill="white") +
...
kp_bubblechart <- 
    ...
    geom_point(aes(size=number_cases), alpha=0.8, na.rm = TRUE)+#, show.legend = FALSE) +
...
for (i in 1:nrow(data_kp_all_n)) {
    ...
    kp_bubblechart <-
        ...
        annotate("text", x = "Personality disorders", y = data_kp_all_n$tech_label[i], label = data_kp_all_n$n[i],
             color = color_cat, fontface =2, size=3, hjust = 0.4, vjust = -1.8)
